  ...
  testthat::expect_true(
    all(
      ion_res$roi_data[, unique(rt)] %between%
        range(MSnbase::rtime(MSnbase::filterFile(MSnExpObj, 1)))
    )
  )
  testthat::expect_true(
    all(
      ion_res$peak_info[, unique(c(mzmed, mzmin, mzmax))] %between%
        range(MSnbase::mz(MSnbase::filterFile(MSnExpObj, 1)))
    )
  )
  testthat::expect_true(
    ion_res$roi_data[, .N] <= {
      MSnbase::filterFile(MSnExpObj, 1) %>%
        MSnbase::spectra() %>%
    ...
    for (msleveli in c(1,2)) {
      for (format_i in c("MSnbase", "MsExperiment", "table")) {
        ...
        msnexp <- switch(
          ...
          "table" = get_rawdata(MsExperiment::readMsExperiment(mzml_path))[, .(file = 1, mslevel, rt, mz, i)]
        )
